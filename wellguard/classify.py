...
def _declared_mode_shift(df: pd.DataFrame, warmup: int) -> int:
    """First index after warmup where declared operating_mode changes to a regime shift."""
    if MODE_COL not in df.columns or len(df) <= warmup + 1:
        return -1
    modes = df[MODE_COL].astype(str).str.lower().to_numpy()
    # Ignore brief quality/transient tags — only deliberate regime labels.
...
def classify(df: pd.DataFrame) -> dict:
    ...
    # 2) gas interference (requires intake when available for cross-channel check)
    if intake_available:
    ...
    # 3) sensor-fault suspected: intake moved, rate/power quiet (needs intake)
    if intake_available:
    ...
    # 4) intake restriction: sustained rate + load decline together
